...
public class DriveTrain extends SwerveDrivetrain {
  ...
  /**
   * Returns a new PID controller that can be used to control the position of the
   * robot chassis in one axis.
   * 
   ...
   * robot chassis.
   * The output will be in radians per second, representing the desired angular
   * velocity.
   * 
  ...
  public static final DrivetrainConfig _config = new DrivetrainConfig(maxMetersPerSecond, .5, 7, 2,
      Units.inchesToMeters(2), 6.75, 2048);
  ...
  private static final WPI_CANSparkMax frontLeftSteerFalcon = new WPI_CANSparkMax(
      Constants.Drivetrain.FRONT_LEFT_STEER_CHANNEL, MotorType.kBrushless);
  private static final CANcoder frontLeftEncoder = new CANcoder(Constants.Drivetrain.FRONT_LEFT_CANCODER_CHANNEL);
  private static final SwerveModule frontLeft = SwerveModule.createFromDriveFalconAndSteeringNeo(frontLeftDriveFalcon,
      frontLeftSteerFalcon, frontLeftEncoder, _config, .1, 0, 0, .8, 0, 0, 1);

  private static final TalonFX frontRightDriveFalcon = new TalonFX(Constants.Drivetrain.FRONT_RIGHT_DRIVE_CHANNEL);
  private static final WPI_CANSparkMax frontRightSteerFalcon = new WPI_CANSparkMax(
      Constants.Drivetrain.FRONT_RIGHT_STEER_CHANNEL, MotorType.kBrushless);
  private static final CANcoder frontRightEncoder = new CANcoder(Constants.Drivetrain.FRONT_RIGHT_CANCODER_CHANNEL);
  private static final SwerveModule frontRight = SwerveModule.createFromDriveFalconAndSteeringNeo(frontRightDriveFalcon,
      frontRightSteerFalcon, frontRightEncoder, _config, .1, 0, 0, .8, 0, 0, 3);

  private static final TalonFX backLeftDriveFalcon = new TalonFX(Constants.Drivetrain.BACK_LEFT_DRIVE_CHANNEL);
  private static final WPI_CANSparkMax backLeftSteerFalcon = new WPI_CANSparkMax(
      Constants.Drivetrain.BACK_LEFT_STEER_CHANNEL, MotorType.kBrushless);
  private static final CANcoder backLeftEncoder = new CANcoder(Constants.Drivetrain.BACK_LEFT_CANCODER_CHANNEL);
  private static final SwerveModule backLeft = SwerveModule.createFromDriveFalconAndSteeringNeo(backLeftDriveFalcon,
      backLeftSteerFalcon, backLeftEncoder, _config, .1, 0, 0, .8, 0, 0, 2);

  private static final TalonFX backRightDriveFalcon = new TalonFX(Constants.Drivetrain.BACK_RIGHT_DRIVE_CHANNEL);
  private static final WPI_CANSparkMax backRightSteerFalcon = new WPI_CANSparkMax(
      Constants.Drivetrain.BACK_RIGHT_STEER_CHANNEL, MotorType.kBrushless);
  private static final CANcoder backRightEncoder = new CANcoder(Constants.Drivetrain.BACK_RIGHT_CANCODER_CHANNEL);
  private static final SwerveModule backRight = SwerveModule.createFromDriveFalconAndSteeringNeo(backRightDriveFalcon,
      backRightSteerFalcon, backRightEncoder, _config, .1, 0, 0, .8, 0, 0, 4);
  ...
  public void periodic() {
    ...
    if (Robot.isReal()) {
      ...
      if (hasTarget) {
        double[] visionPose = NetworkTableInstance.getDefault().getTable("limelight").getEntry("botpose_wpiblue")
        ...
        if (bufferSize >= 6) {
          poseEstimator.addVisionMeasurement(new Pose2d(visionPose[0], visionPose[1], getPose2d().getRotation()),
    ...
    // log the current
    // m_CurrentBLLogger.logEntry(backLeftDriveFalcon.getSupplyCurrent(),
  ...
  public ScoringArea pickConeScoringArea() {
    ScoringArea[] cubes = { ScoringArea.CONE1, ScoringArea.CONE2, ScoringArea.CONE3, ScoringArea.CONE4,
        ScoringArea.CONE5, ScoringArea.CONE6 };
    // iterate
    ScoringArea selected = cubes[0];
    double selectedDistance = Double.MAX_VALUE;
    for (ScoringArea scoringArea : cubes) {
